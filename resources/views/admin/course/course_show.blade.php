...
<body>
    ...
    <div class="content">
        ...
        @foreach($course as $ca)
        <div class="course-card">
            <h3>{{ $ca->title }}</h3>
            <p>{{ $ca->description }}</p>

            <div class="course-actions">
                <a href="{{ route('course.show', $ca->id) }}">🔍 Details</a>
                <a href="{{ route('course.edit', $ca->id) }}">✏️ Edit</a>

                <form action="{{ route('course.destroy', $ca->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
